...
- `frontend/src/app/gex/page.tsx` — the real gamma-exposure page: reads `/api/gex` (incrementally, via `?since=`) plus `/api/market?history=true` for the spot line.
- `frontend/src/app/spx-gamma/page.tsx` — **a mockup, not a live view.** The GEX bars and historical dots are hardcoded arrays at the top of the file (strikes 7325–7500); only the spot line is real, from `/api/market`. It never touches `/api/gex`, and its `SyntheticDataBadge` is accurate — that badge no longer belongs on `/gex`.
